class Calculate
{
    ...
    {
        // Проверка расстояния и веса на положительное число
        if (weight <= 0)
        {
        ...
        // Рассчёт расстояния
        // 3-й вариант требовал, чтобы за каждый километр свыше 1000 была скидка в 20%.
        if (distance <= 1000)
        {
            cost += (decimal)distance * 0.5m;
        }
        else
        {
            cost += 500m + (decimal)(distance - 1000) * 0.4m;
        }
        ...
        // Определение коэффициента за тип товара
        ...
        {
            throw new ArgumentException("Invalid DeliveryType");
        }
